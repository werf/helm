...
func (s *ReleaseServer) execHook(hs []*release.Hook, name, namespace, hook string, timeout int64) error {
	...
	for _, h := range executingHooks {
		if err := s.deleteHookIfShouldBeDeletedByExistsPolicy(h, hooks.HookDelete, name, namespace, hook, kubeCli); err != nil {
			return err
		}
...
func (s *ReleaseServer) deleteHookIfShouldBeDeletedByDeletePolicy(h *release.Hook, policy string, name, namespace, hook string, kubeCli environment.KubeClient) error {
	return s.deleteHookIfShouldBeDeleted(h, policy, name, namespace, hook, kubeCli, hookHasDeletePolicy)
}

func (s *ReleaseServer) deleteHookIfShouldBeDeletedByExistsPolicy(h *release.Hook, policy string, name, namespace, hook string, kubeCli environment.KubeClient) error {
	return s.deleteHookIfShouldBeDeleted(h, policy, name, namespace, hook, kubeCli, hookHasExistsPolicy)
}

func (s *ReleaseServer) deleteHookIfShouldBeDeleted(h *release.Hook, policy string, name, namespace, hook string, kubeCli environment.KubeClient, hookHasPolicy func(h *release.Hook, policyName string) bool) error {
	b := bytes.NewBufferString(h.Manifest)
	if hookHasPolicy(h, policy) {
...
func hookHasExistsPolicy(h *release.Hook, policy string) bool {
	if dp, ok := existsPolices[policy]; ok {
		for _, v := range h.ExistsPolicies {
			if dp == v {
				return true
			}
		}
